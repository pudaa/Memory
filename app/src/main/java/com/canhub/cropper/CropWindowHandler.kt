...
internal class CropWindowHandler {
  ...
  /** Minimum height in pixels that the crop window can get. */
  fun getMinCropHeight() = mMinCropWindowHeight.coerceAtLeast(mMinCropResultHeight / mScaleFactorHeight)
  ...
  fun getMaxCropHeight() = mMaxCropWindowHeight.coerceAtMost(mMaxCropResultHeight / mScaleFactorHeight)
  ...
  private fun getRectangleVerticalOnlyPressedMoveType(
    ...
    return when {
      ...
      isCenterMoveEnabled && isInCenterTargetZone(x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom) -> {
        CropWindowMoveHandler.Type.CENTER
      }
      else -> getOvalPressedMoveType(x, y, isCenterMoveEnabled)
    }
