...
class CropImageView @JvmOverloads constructor(
  ...
  public override fun onRestoreInstanceState(state: Parcelable) {
    if (state is Bundle) {
      ...
      if (bitmapLoadingWorkerJob == null && imageUri == null && originalBitmap == null && mImageResource == 0) {
        ...
        var uri: Uri? = state.getParcelable("LOADED_IMAGE_URI") as? Uri
        if (uri != null) {
          ...
          if (resId > 0) {
            ...
            @Suppress("DEPRECATION")
            uri = state.getParcelable("LOADING_IMAGE_URI") as? Uri
        ...
        @Suppress("DEPRECATION")
        val initialCropRect: Rect? = state.getParcelable("INITIAL_CROP_RECT") as? Rect
        ...
        @Suppress("DEPRECATION")
        val cropWindowRect: RectF? = state.getParcelable("CROP_WINDOW_RECT") as? RectF
  ...
  enum class ScaleType {
    /** Scale the image uniformly (maintain the image's aspect ratio) to fit in crop image
     * view.<br></br>
  ...
  enum class RequestSizeOptions {
     ...
     * NOTE: resulting image will not be exactly requested width/height see: [Loading Large Bitmaps
     * Efficiently](http://developer.android.com/training/displaying-bitmaps/load-bitmap.html).
     */
    SAMPLING,
  ...
  fun interface OnSetCropOverlayMovedListener {
    /** Called when the crop overlay moved listener is called and inProgress is false.
     *
     * [rect] The rect coordinates of the cropped overlay
     */
  ...
  init {
    // 【定制】统一使用默认配置，由 setImageCropOptions() 注入主题；
    // 不再从 Activity intent / XML attrs 解析配置（移除对 CropImage 与 styleable 的依赖）。
