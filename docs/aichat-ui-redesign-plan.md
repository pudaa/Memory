...
|---|---|---|---|
| 标题栏返回 | `back.png` | 位图 | `arrow_back_ios_new` |
...
- **录音态降噪**：不再整条铺满红底。改为「圆形红点指示 + 文字转红 + 波形转红」，
  容器底色保持浅底，避免大面积高饱和
- **布局平衡**：语音模式下在发送按钮原位置放一个 40dp 圆形**录音主按钮**
  （话筒图标，`theme_primary` 底），补上右侧缺口；同时隐藏场景按钮（录音时无意义）
- 录音区仍保持整条可点，主按钮触发同一录音逻辑（交互向后兼容）

> 注意：其中「新增录音主按钮」与「隐藏场景按钮」属于**交互层面**的调整，
> 不只是视觉。若你希望严格限定在美术范围，可退化为仅做波形、卡片、配色三项。
